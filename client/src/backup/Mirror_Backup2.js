...
import sad from './res/sad.png';
import surprised from './res/surprised.png';
...
class Mirror extends Component {
	...
	onMessage(evt){
		...
		// console.log('centerX: ', bufferObj['x_33'], 'centerY: ', bufferObj['y_33']);
		// console.log('vidWidth: ', 640, 'vidHeight: ', 480); //	int cam_width = 640; int cam_height = 480;

		this.setState({
			...newState, clipPoints,
			...{
				centerX: bufferObj['x_33'],
				centerY: camHeight - bufferObj['y_33']
			},
			... {
				faceWidth: (bufferObj['x_16'] - bufferObj['x_0']) / camWidth
			}
		}); // Y axis is flipped
	}
	...
	renderCanvas() {
		...
		uniforms.faceWidth.value = this.state.faceWidth;
	...
	onCamMetaDataLoaded() {
		...
		uniforms.resolution.value.x = camWidth;
		uniforms.resolution.value.y = camHeight;
		...
		// FROG DITHER DISPLAY ///////
		//////////////////////////////
		shaderMaterialDither = new THREE.ShaderMaterial({
				uniforms: uniforms,
				...
				void main() {
					...
					float grayscale = dot(texture2D(texture1, vUv).rgb, lum);
					vec3 rgb = texture2D(texture1, vUv).rgb;
					vec2 xy = gl_FragCoord.xy * scale;
					int x = int(mod(xy.x, 5.0));
					int y = int(mod(xy.y, 5.0));
					vec3 finalRGB;
					...
					finalRGB = mix()
					if (texture3 == 0.){
						finalRGB = vec3(find_closest(x, y, grayscale));
					}
					else if (texture3 == 1.){
		...
		shaderMaterialFlipDot = new THREE.ShaderMaterial({
				...
				float display(vec2 uv, float t){
					...
					vec2 uvOrig = uv / vec2(CELL_SIZE * 1.5, CELL_SIZE);
					t *= 0.01;
					
					vec2 center = vec2(640.0/2.0, 360.0/2.0) + vec2(640.0/2.0*sin(-t*3.0), 360.0/2.0*cos(-t*3.0));

					float color1 = (sin(dot(uvScaled.xy, vec2(sin(t*3.0), cos(t*3.0)))*0.02 + t*3.0) + 1.0)/2.0;
					float color2 = (cos(length(uvScaled.xy - center)*0.03) + 1.0)/2.0;
					float color = (color1 + color2)/2.0;

					//Make sure there's none around face
					vec2 location;
					float distance = sqrt(pow((centerX / resolution.x) - uvOrig.x, 2.) + pow((centerY  / resolution.y) - uvOrig.y, 2.));
					float scale = 0.8;
					if (distance < faceWidth * scale) {
						return 0.;
					}
					else {
						return (cos(M_PI*color/0.5 + t*3.0) + 1.0)/2.0; //[-0.5, 0.5]
					}
				}
				...
				void main() {
					...
					vec4 greenBorder = vec4((1. - aa) * vec3(0., 1., 0.), 1.);
		...
		wiremirror = new THREE.Mesh(geometry, shaderMaterialDither);
	...
	// 			paddingRight: 40
		...
		// let counter = 0;
		let smileTexture = new THREE.TextureLoader().load( smiley );
		let sadTexture = new THREE.TextureLoader().load( sad );
		let surprisedTexture = new THREE.TextureLoader().load( surprised );
		var lastEmotion;
		setInterval(() => {
			let startDelight = 0;
			let delightInterval = setInterval(() => {
				shaderMaterialDither.uniforms.texture3.value = startDelight;
			}, 50)
			startDelight += .1;
			if (startDelight === 1.) clearInterval(delightInterval);
			let curEmotion = defaultEmotion;
			if (curEmotion !== lastEmotion && shaderMaterialFlipDot) {
				lastEmotion = curEmotion;
				switch(curEmotion){
					case 'Delight':
						shaderMaterialFlipDot.uniforms.texture2.value = smileTexture;
						break;
					case 'Contemplation':
						shaderMaterialFlipDot.uniforms.texture2.value = sadTexture;
						// shaderMaterialDither.uniforms.texture3.value = 2.0;
						break;
					case 'Surprise':
						shaderMaterialFlipDot.uniforms.texture2.value = surprisedTexture;
						// shaderMaterialDither.uniforms.texture3.value = 3.0;
						break;
					default:
						// shaderMaterialDither.uniforms.texture3.value = 0.;
